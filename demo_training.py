...
for mood, genres in mood_map.items():
    target = set(genres)
    mask = movies["genres"].apply(lambda g: len({s.strip() for s in g.split(",") if s.strip()} & target) > 0)
    count = mask.sum()
...
# Filter candidates
mask = movies["genres"].apply(lambda g: len({s.strip() for s in g.split(",") if s.strip()} & target_genres) > 0)
...
for idx in candidates:
    ...
    g = {s.strip() for s in movies.loc[idx, "genres"].split(",") if s.strip()}
