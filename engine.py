...
class MoodRecommender:
    def __init__(self, movies_df, ratings_df=None):
        ...
        # Ensure genres column is string to avoid runtime errors
    ...
    def get_recommendations(self, mood, user_id=None, top_n=10):
        ...
        def genre_match(g_str):
            genres_in_movie = {g.strip() for g in g_str.split(',') if g.strip()}
            return len(genres_in_movie & target_genres) > 0
        ...
        for idx in c_indices:
            g = {s.strip() for s in self.movies.loc[idx, 'genres'].split(',') if s.strip()}
