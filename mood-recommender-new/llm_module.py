...
def classify_mood(user_text, use_llm=False, api_key=None):
    """
    Classify free-text user input into a mood category.

    Args:
        user_text: Raw user input string
        use_llm: Set True to use OpenAI API (requires key)
        api_key: OpenAI API key (or set OPENAI_API_KEY env var)

    Returns:
        dict: {mood_category, confidence, explanation, preferred_genres, avoid_genres}
    """
