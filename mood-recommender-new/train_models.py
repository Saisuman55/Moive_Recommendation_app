...
def validate_mood_mapping(movies_df, mood_map, output_dir="models"):
    ...
    for mood, genres in mood_map.items():
        ...
        mask = movies_df["genres"].apply(
            lambda g: len(set(g.split(",")) & target) > 0
        )
        count = mask.sum()
        coverage[mood] = {
...
def evaluate_hybrid(movies_df, ratings_train, ratings_test, cosine_sim, mood_map, top_n=10):
    ...
    Metrics: HR@K (Hit Ratio), RMSE proxy, Coverage
    """
    print("\n🔧 [4/4] Evaluating Hybrid Model...")

    movie_indices = pd.Series(movies_df.index, index=movies_df["id"]).drop_duplicates()

    # For each test rating, check if the movie appears in top-N recommendations
    # when using the user's train history + a random mood from their genres
    hits = 0
    total = 0

    test_users = ratings_test["user_id"].unique()[:100]  # sample for speed
    ...
    for user_id in test_users:
        ...
        liked_idx = [movie_indices[mid] for mid in liked_train if mid in movie_indices]

        for _, row in user_test.iterrows():
            target_movie = row["movie_id"]
            if target_movie not in movie_indices:
                continue

            target_idx = movie_indices[target_movie]

            # Simulate: pick a mood based on target movie's genres
            target_genres = set(movies_df.loc[target_idx, "genres"].split(","))
            best_mood = "chill"
            best_overlap = 0
            for mood, genres in mood_map.items():
            ...
            # Get candidates for that mood
            target_mood_genres = set(mood_map[best_mood])
            mask = movies_df["genres"].apply(
                lambda g: len(set(g.split(",")) & target_mood_genres) > 0
            )
            ...
            for idx in candidates:
                ...
                # Genre match
                g = set(movies_df.loc[idx, "genres"].split(","))
